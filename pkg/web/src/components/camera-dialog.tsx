...
export function useLikelyHasCamera() {
  ...
  useEffect(() => {
    // iOS Safari in insecure contexts exposes navigator.mediaDevices without
    // enumerateDevices, so probe for the function itself before calling it.
 ...
 * sm and up.
 *
 * The viewport and the captured image are both the centermost square of the
 * camera frame — the square viewport discourages rotating the device, and
 * `onCapture` always receives a square JPEG File.
 */
export function CameraDialog({
  ...
  // Center-square crop, matching what the object-cover square viewport shows.
  function takePhoto() {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent
        ...
        className="top-0 left-0 flex h-dvh w-full max-w-none translate-x-0 translate-y-0 flex-col gap-0 rounded-none bg-black p-0 text-white sm:top-1/2 sm:left-1/2 sm:h-auto sm:max-w-lg sm:-translate-x-1/2 sm:-translate-y-1/2 sm:gap-6 sm:rounded-4xl sm:bg-popover sm:p-6 sm:text-popover-foreground"
      >
        ...
        <div className="flex min-h-0 flex-1 items-center justify-center sm:flex-none">
          {/* Square viewport: object-cover shows exactly the center square
              that takePhoto crops. max-w keeps it on-screen when the viewport
              is short (e.g. landscape phones). */}
          <div className="relative aspect-square w-full max-w-[calc(100dvh-16rem)] overflow-hidden bg-black sm:max-w-none sm:rounded-2xl">
            {cameraError ? (
