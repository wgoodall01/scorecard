#!/usr/bin/env bun
// Eval CLI for the card_metadata agent — real vision-model calls against the
...
//                 --fixtures meta-01                          # a specific slice
//   ./eval.ts score                                           # re-score results/latest
//   ./eval.ts score 2026_07_18__14_07_37                      # re-score a past run
//
...
import { extractCardMetadata } from "../agent";
import { CardMetadata, type CardMetadataSchema } from "../schema";
...
// Local-time run stamp: "2026_07_18__14_05_33"
...
function loadLabels(): Map<string, CardMetadataSchema> {
  const labels = new Map<string, CardMetadataSchema>();
  ...
  for (const label of readdirSync(scorecardDir)) {
    ...
    const parsed = CardMetadata.safeParse(JSON.parse(readFileSync(path, "utf-8")));
    if (!parsed.success) {
...
function scoreRun(runDir: string) {
  ...
  for (const fixture of readdirSync(runDir).sort()) {
    ...
    for (const slug of readdirSync(join(runDir, fixture)).sort()) {
      ...
      const graded = score(CardMetadata.parse(output), expected);
  ...
  args: {
    models: option({
      ...
      description:
        ...
        "gemini-3.5-flash, claude-sonnet-5 (each @low).",
    }),
    fixtures: option({
      type: optional(string),
      long: "fixtures",
      description: "Comma-separated fixture labels (e.g. meta-01). Default: all.",
    }),
  },
  handler: async (args) => {
    loadGatewayEnv();
    ...
    await pMap(
      ...
      async ({ spec, fixture }) => {
        ...
        try {
          const data = await extractCardMetadata({
...
const app = subcommands({
  name: "eval",
  description: "Run and grade card_metadata agent evals.",
