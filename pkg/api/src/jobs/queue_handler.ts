...
async function runJob(env: Env["Bindings"], id: string) {
  ...
  // Already terminal — a duplicate delivery or a job someone else finished.
  // Never re-run (extract_score would re-spend the vision call).
  ...
  try {
    ...
    await db.update(job).set({ state: "ok", result, error: null }).where(eq(job.id, id));
  } catch (error) {
    console.error("Job failed", { id, jobType: row.jobType, error });
    await db
      .update(job)
      .set({ state: "error", result: null, error: toJobError(error) })
      .where(eq(job.id, id));
  }
}
